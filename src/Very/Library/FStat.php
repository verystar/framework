...
class FStat {
    ...
    public $environ = 'dev'; //执行环境 dev开发 test测试  pro线上
    public $use_db = 'stat';
        ...
        foreach ($stat_arr as $dbf => $stats) {
            ...
            foreach ($stats as $k => $v) {
                if ($this->isExist($k, 'v4', $dbf)) {
                    ...
                    $this->db()->execute(sprintf($sql2, $dbf), array('num' => $v['num'], 'v1' => $v['v1'], 'v2' => $v['v2'], 'v3' => $v['v3'], 'v4' => $v['v4'], 'md5' => $k, 'md5_v2' => $v['md5_v2'], 'md5_v3' => $v['md5_v3'], 'add_time' => $v['time']));
                }
            }
        }
    }
    ...
    public function statSumHour($db_prefix, $add_time = NULL) {
        ...
        foreach ($hour_arr as $v) {
            ...
            if ($this->isExist($v['md5_v3'], 'v3', $db_prefix)) {
                ...
                $this->db()->execute($sql2, array('num' => $v['num'], 'v1' => $v['v1'], 'v2' => $v['v2'], 'v3' => $v['v3'], 'md5' => $v['md5_v3'], 'add_time' => $add_time));
            }
        }
